...
#define DAQ_PKT_FLAG_OPAQUE_IS_VALID                                                               \
    0x0001 /* The DAQ module actively set the opaque value in the DAQ packet header. */
#define DAQ_PKT_FLAG_NOT_FORWARDING                                                                \
    0x0002 /* The DAQ module will not be actively forwarding this packet                           \
               regardless of the verdict (e.g, Passive or Inline Tap interfaces). */
#define DAQ_PKT_FLAG_PRE_ROUTING                                                                   \
    0x0004 /* The packet is being routed via us but packet modifications                           \
               (MAC and TTL) have not yet been made. */
#define DAQ_PKT_FLAG_IGNORE_VLAN 0x0008 /* Ignore vlan tags in the packet */
#define DAQ_PKT_FLAG_FLOWID_IS_VALID                                                               \
    0x0010 /* The DAQ module actively set the flow ID value in the DAQ packet header. */
...
#define DAQ_PKT_FLAG_HA_STATE_AVAIL                                                                \
    0x0100 /* HA State is availble for the flow this packet is associated with. */
...
#define DAQ_PKT_FLAG_REV_FLOW                                                                      \
    0x2000 /* The packet is going the reverse direction of the flow initiator.*/
#define DAQ_PKT_FLAG_DEBUG_ENABLED                                                                 \
    0x4000 /* The packet has been flagged for debugging by the lower layer. */
#define DAQ_PKT_FLAG_SIGNIFICANT_GROUPS                                                            \
...
#define DAQ_NAPT_INFO_FLAG_DIP_V6 0x02 /* The destination address is IPv6 */
    ...
    /* Decoded packet information parsed from the Packet message's data.  Currently, all fields
       refer to the first protocol of each layer encountered (no information is conveyed about
       encapsulated duplicate protocols like IP-in-IP).  The offsets for layers not found are set to
    ...
    typedef union _daq_pkt_decode_flags
    {
        ...
        struct
        {
            ...
            uint32_t checksum_error : 1; /* One or more checksum errors were encountered during
                                            parsing. */
...
#define DAQ_FS_FLAG_LOGGING_OPTIONAL 0x2
    ...
    typedef enum
    {
        ...
        DAQ_VERDICT_REPLACE,   /* Pass a packet that has been modified in-place. (No resizing
                                  allowed!) */
        ...
        DAQ_VERDICT_BLACKLIST, /* Block the packet and block all future packets in the same flow
                                  systemwide. */
        DAQ_VERDICT_IGNORE, /* Pass the packet and fastpath all future packets in the same flow for
                               this application. */
        MAX_DAQ_VERDICT
    } DAQ_Verdict;
...
#define DAQ_CAPA_REPLACE                                                                           \
    0x00000002 /* can replace/modify packet data (up to the original data size) */
...
#define DAQ_CAPA_DEVICE_INDEX                                                                      \
    0x00000100 /* can consistently fill the device_index field in DAQ_PktHdr */
...
#define DAQ_CAPA_DECODE_GENEVE 0x00080000 /* decodes and tracks flows within Geneve. */
    ...
    typedef enum
    {
        ...
        LAST_BUILTIN_DIOCTL_CMD =
            1024, /* End of reserved space for "official" DAQ ioctl commands.
     ...
     * Description: Given a device name, query the index (as used in ingress/egress_index)
     * associated with it. Argument: DIOCTL_QueryDeviceIndex
     */
    typedef struct
    {
     ...
     * Description: Add verdict reason and tracing text to the packet associated with the DAQ
     * message. Argument: DIOCTL_SetPacketTraceData
     */
    typedef struct
    {
     ...
     * Description: Retrieve unmodified TCP options from the SYN for the flow associated with the
     * DAQ message. Argument: DIOCTL_GetFlowScrubbedTcp
     *
     * Command: DIOCTL_GET_FLOW_TCP_SCRUBBED_SYN_ACK
     * Description: Retrieve unmodified TCP options from the SYN-ACK for the flow associated with
     * the DAQ message. Argument: DIOCTL_GetFlowScrubbedTcp
     */
    typedef enum
    {
 ...
 * Description: Create an expected flow in the dataplane based on an N-tuple with some optional
 * wildcards. Argument: DIOCTL_CreateExpectedFlow
 */
...
#define DAQ_EFLOW_BIDIRECTIONAL 0x08 /* create expected flow in both direction */
     ...
     * Description: Directly inject L5 payload data on a flow relative to the reference message. The
     * module should handle any packetizing necessary to get the data onto the wire. Argument:
...
#define DAQ_DIR_BOTH 2    // Both forward and reverse injection
    ...
    typedef struct
    {
        ...
        uint64_t conn_meta_null_counters; /* Number of times packet conn meta was null in last 5
                                             minutes */
...
#define DAQ_INSTANCE_API_STRUCT(fname)                                                             \
    struct                                                                                         \
    {                                                                                              \
        daq_module_##fname##_func func;                                                            \
        void* context;                                                                             \
    } fname
...
#define DAQ_MODULE_API_VERSION 0x00030001
    ...
    struct driver_api_st
    {
        ...
        /* Inject a new message going either the same or opposite direction as the specified
         * message. */
