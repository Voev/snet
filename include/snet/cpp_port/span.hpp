...
// Use C++20 span if available and requested:
...
#define span_CONTRACT_CHECK(type, cond)                                                                                \
    cond                                                                                                               \
        ? static_cast<void>(0)                                                                                         \
        : cpp::span_lite::detail::report_contract_violation(span_LOCATION(__FILE__, __LINE__) ": " type " violation.")
...
#define span_REQUIRES_T(VA) , typename = typename std::enable_if<(VA), cpp::span_lite::detail::enabler>::type
...
template <typename ElementType, cpp::span_lite::extent_t Extent>
class tuple_size<cpp::span<ElementType, Extent> > : public integral_constant<size_t, static_cast<size_t>(Extent)>
{
};
...
class tuple_size<cpp::span<ElementType, cpp::dynamic_extent> >;

// std::tuple_element<>:

template <size_t I, typename ElementType, cpp::span_lite::extent_t Extent>
class tuple_element<I, cpp::span<ElementType, Extent> >
{
public:
#if span_HAVE(STATIC_ASSERT)
    static_assert(Extent != cpp::dynamic_extent && I < Extent,
...
template <size_t I, typename ElementType, cpp::span_lite::extent_t Extent>
span_constexpr ElementType& get(cpp::span<ElementType, Extent>& spn) span_noexcept
{
#if span_HAVE(STATIC_ASSERT)
    static_assert(Extent != cpp::dynamic_extent && I < Extent, "get<>(span): dynamic extent or index out of range");
#endif
    return spn[I];
}

template <size_t I, typename ElementType, cpp::span_lite::extent_t Extent>
span_constexpr ElementType const& get(cpp::span<ElementType, Extent> const& spn) span_noexcept
{
#if span_HAVE(STATIC_ASSERT)
    static_assert(Extent != cpp::dynamic_extent && I < Extent, "get<>(span): dynamic extent or index out of range");
#endif
