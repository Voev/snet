...
inline constexpr void assert_equal_byte_lengths(R0&& r0, Rs&&... rs) requires(sizeof...(Rs) > 0) {
...
template <typename T>
concept reservable_container = container<T>&& requires(T& c, typename T::size_type s) {
    c.reserve(s);
};
