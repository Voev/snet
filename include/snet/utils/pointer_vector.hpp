...
/**
 * @brief A specialization of Copier to facilitate the safe copying of
 * polymorphic objects via clone() method.
 * @tparam T The type of object to copy.
 */
template <class T>
...
public:
    ...
    /**
     * Const iterator object that is used for iterating all elements in a
     * constant vector
     */
     ...
     * All elements inside the copied vector are duplicates and the originals
     * remain unchanged.
     * @param[in] other The vector to copy from.
     * @remarks As the vector is copied via deep copy, all pointers obtained
     * from the copied vector reference the duplicates and not the originals.
     */
    ...
    /**
     * Move constructor. All elements along with their ownership is transferred
     * to the new vector.
    ...
    /**
     * A copy assignment operator. Replaces the contents with a copy of the
     * contents of other. See copy constructor for more information on the
     * specific copy procedure.
     * @param[in] other The vector to copy from.
    ...
    PointerVector& operator=(const PointerVector& other)
    {
        ...
        // If an exception is thrown during the copy operation, restore old
        // values and rethrow.
    ...
    /**
     * A move assignment operator. Replaces the contents with those of other via
     * move semantics. The other vector is left empty.
     * @param[in] other The vector to move from.
     * @return A reference to the current object.
     */
    PointerVector& operator=(PointerVector&& other) noexcept
    {
        ...
        m_Vector = std::move(other.m_Vector);
        // Explicitly clear the other vector as the standard only guarantees an
        // unspecified valid state after move.
        other.m_Vector.clear();
        return *this;
    }
     ...
     */
    void pushBack(std::nullptr_t element,
                  bool freeElementOnError = true) = delete;

    /**
     * Add a new (pointer to an) element to the vector
     * @param[in] element A pointer to an element to assume ownership of.
     * @param[in] freeElementOnError If set to true, the element is freed if an
     * exception is thrown during the push.
    ...
    /**
     * Add a new element to the vector that has been managed by an unique
     * pointer.
    ...
    void pushBack(std::unique_ptr<T> element)
    {
        ...
        // Release is called after the raw pointer is already inserted into the
        // vector to prevent a memory leak if push_back throws.
     ...
     * @return A const iterator object pointing to the first element of the
     * vector
     */
     ...
     * @return A const iterator object pointing to the last element of the
     * vector
     */
    ...
    /**
     * Removes an element from the vector and transfers ownership to the
     ...
     * The iterator is shifted to the following element after the detach
     * completes.
     * @return An unique pointer that holds ownership of the detached element.
     */
...
private:
    ...
    static std::vector<T*> deepCopyUnsafe(std::vector<T*> const& origin)
    {
        ...
        // Allocate the vector initially to ensure no exceptions are thrown
        // during push_back.
     ...
     * @remarks The vector's contents are not cleared and will point to invalid
     * locations in memory.
     */
