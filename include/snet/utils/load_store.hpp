...
#include <snet/utils/typecast.hpp>
...
inline constexpr uint8_t get_byte(T input) requires(B < sizeof(T)) {
...
inline constexpr uint32_t make_uint32(uint8_t i0, uint8_t i1, uint8_t i2, uint8_t i3) {
    return (
        (static_cast<uint32_t>(i0) << 24) | (static_cast<uint32_t>(i1) << 16) | (static_cast<uint32_t>(i2) << 8) |
...
make_uint64(uint8_t i0, uint8_t i1, uint8_t i2, uint8_t i3, uint8_t i4, uint8_t i5, uint8_t i6, uint8_t i7) {
    return (
        (static_cast<uint64_t>(i0) << 56) | (static_cast<uint64_t>(i1) << 48) | (static_cast<uint64_t>(i2) << 40) |
 ...
 * compile time.
 */
template <std::endian endianness, std::unsigned_integral OutT, ranges::contiguous_range<std::uint8_t> InR>
 ...
 */
template <std::endian endianness, concepts::unsigned_integral_strong_type OutT, ranges::contiguous_range<uint8_t> InR>
inline constexpr OutT load_any(InR&& in_range) {
...
requires(std::is_enum_v<OutT>&& std::unsigned_integral<std::underlying_type_t<OutT>>) inline constexpr OutT
...
template <std::endian endianness, typename OutT, ranges::contiguous_range<uint8_t> InR, unsigned_integralish... Ts>
    ...
    ((std::same_as<snet::detail::AutoDetect, OutT> && all_same_v<Ts...>) ||
     (unsigned_integralish<OutT> && all_same_v<OutT, Ts...>)) inline constexpr void load_any(InR&& in, Ts&... outs) {
 ...
 */
template <
    std::endian endianness, typename OutT, ranges::contiguous_output_range OutR, ranges::contiguous_range<uint8_t> InR>
requires(
    unsigned_integralish<std::ranges::range_value_t<OutR>> &&
    (std::same_as<snet::detail::AutoDetect, OutT> ||
     std::same_as<OutT, std::ranges::range_value_t<OutR>>)) inline constexpr void load_any(OutR&& out, InR&& in) {
...
requires(
    std::same_as<snet::detail::AutoDetect, OutT> ||
    ((ranges::statically_spanable_range<OutT> || concepts::resizable_container<OutT>)&&unsigned_integralish<
        typename OutT::value_type>)) inline constexpr auto load_any(InR&& in_range) {
    auto out = []([[maybe_unused]] const auto& in) {
        if constexpr (std::same_as<snet::detail::AutoDetect, OutT>) {
            ...
            using type =
               ...
               std::conditional_t<extent == 8, uint64_t, void>>>>;
                ...
                static_assert(
                    !std::is_void_v<type>, "Cannot determine the output type based on a statically sized bytearray "
                                           "with length other than those: 1, 2, 4, 8");
            ...
            //BOTAN_ARG_CHECK(
            //    in_bytes % out_elem_bytes == 0, "Input range is not word-aligned with the requested output range");
...
requires(
    ...
    ((std::same_as<snet::detail::AutoDetect, OutT> && all_same_v<Ts...>) ||
     (unsigned_integralish<OutT> &&
      all_same_v<OutT, Ts...>))) inline constexpr void load_any(const uint8_t in[], Ts&... outs) {
...
requires(std::same_as<snet::detail::AutoDetect, OutT> || std::same_as<T, OutT>) inline constexpr void load_any(
    T out[], const uint8_t in[], size_t count) {
 ...
 */
template <typename OutT = snet::detail::AutoDetect, typename... ParamTs>
 ...
 */
template <
    std::endian endianness, concepts::unsigned_integral_strong_type InT, ranges::contiguous_output_range<uint8_t> OutR>
inline constexpr void store_any(InT in, OutR&& out_range) {
...
requires(std::is_enum_v<InT>&& std::unsigned_integral<std::underlying_type_t<InT>>) inline constexpr void store_any(
    InT in, OutR&& out_range) {
 ...
 */
template <
    std::endian endianness, typename InT, ranges::contiguous_output_range<uint8_t> OutR, unsigned_integralish... Ts>
    requires(sizeof...(Ts) > 0) &&
    ((std::same_as<snet::detail::AutoDetect, InT> && all_same_v<Ts...>) ||
     (unsigned_integralish<InT> && all_same_v<InT, Ts...>)) inline constexpr void store_any(OutR&& out, Ts... ins) {
 ...
 */
template <
    std::endian endianness, typename InT, ranges::contiguous_output_range<uint8_t> OutR, ranges::spanable_range InR>
requires(
    std::same_as<snet::detail::AutoDetect, InT> ||
    std::same_as<InT, std::ranges::range_value_t<InR>>) inline constexpr void store_any(OutR&& out, InR&& in) {
    ranges::assert_equal_byte_lengths(out, in);
...
requires std::same_as<snet::detail::AutoDetect, InT> inline constexpr void store_any(T in, OutR&& out_range) {
...
requires(
    std::same_as<snet::detail::AutoDetect, OutR> || (ranges::statically_spanable_range<OutR> && std::default_initializable<OutR>) ||
    concepts::resizable_byte_buffer<OutR>) inline constexpr auto store_any(InR&& in_range) {
    auto out = []([[maybe_unused]] const auto& in) {
        if constexpr (std::same_as<snet::detail::AutoDetect, OutR>) {
            if constexpr (ranges::statically_spanable_range<InR>) {
                constexpr size_t bytes = decltype(std::span{in})::extent * sizeof(std::ranges::range_value_t<InR>);
                return std::array<uint8_t, bytes>();
            } else {
                static_assert(
                    !std::same_as<snet::detail::AutoDetect, OutR>, "cannot infer a suitable result container type from the given "
...
requires all_same_v<Ts...> inline constexpr auto store_any(Ts... ins) {
...
requires(std::same_as<snet::detail::AutoDetect, InT> || std::same_as<T, InT>) inline constexpr void store_any(T in, uint8_t out[]) {
...
template <std::endian endianness, typename InT, unsigned_integralish T0, unsigned_integralish... Ts>
    requires(std::same_as<snet::detail::AutoDetect, InT> || std::same_as<T0, InT>) &&
    all_same_v<T0, Ts...> inline constexpr void store_any(uint8_t out[], T0 in0, Ts... ins) {
 ...
 * See the documentation of this file for more details.
 */
template <typename ModifierT = snet::detail::AutoDetect, typename... ParamTs>
