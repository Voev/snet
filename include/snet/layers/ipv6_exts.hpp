...
public:
    ...
    /**
     * @return A pointer to the next header or nullptr if the extension is the
     * last one
     */
...
public:
    ...
    struct ipv6_frag_header
    {
         ...
         * part of the original packet plus 1-bit indicating if more fragments
         * will follow */
    ...
    /**
     * A c'tor for creating a new IPv6 fragmentation extension object not
     * bounded to a packet. Useful for adding new extensions to an IPv6 layer
     * with IPv6Layer#addExtension()
     ...
     * points directly to the data, so every change will modify the actual
     * packet data
    ...
    /**
     * @return True if the "more fragments" bit is set, meaning more fragments
     * are expected to follow this fragment
     */
...
public:
    /**
     * @class IPv6Option
     * A class representing a Type-Length-Value (TLV) options that are used
     * inside Hop-By-Hop and Destinations IPv6 extensions. This class does not
     * create or modify IPv6 option records, but rather serves as a wrapper and
    ...
    public:
        ...
        /**
         * A c'tor for this class that gets a pointer to the option raw data
         * (byte array)
         * @param[in] optionRawData A pointer to the attribute raw data
         */
        explicit IPv6Option(uint8_t* optionRawData)
    ...
    public:
        /**
         * A c'tor for building IPv6 TLV options which their value is a byte
         * array. The IPv6Option object can later be retrieved by calling
         * build()
         * @param[in] optType IPv6 option type
         * @param[in] optValue A buffer containing the option value. This buffer
        ...
        /**
         * Build the IPv6Option object out of the parameters defined in the
         * c'tor
     ...
     * doesn't exist or (3) the input option is nullptr
     */
...
public:
    ...
    struct ipv6_routing_header
    {
        /** Next header type */
        uint8_t nextHeader;
        /** The length of this header, in multiples of 8 octets, not including
         * the first 8 octets */
        uint8_t headerLen;
        /** A value representing the routing type */
        uint8_t routingType;
        /** Number of nodes this packet still has to visit before reaching its
         * final destination */
     ...
     * additional routing data for this extension. Notice this buffer is
     * read-only and its content isn't modified
     ...
     * pointer points directly to the data, so every change will modify the
     * actual packet data
    ...
    /**
     * @return A pointer to the buffer containing the additional routing data
     ...
     * where the request IPv6 address begins. Also, even if there is only one
     * IPv6 address in this buffer, sometimes it isn't written in the beginning
     * of the buffer, so the offset points to where the IPv6 address begins.
     * This is an optional parameter and the default offset is 0
...
public:
    ...
    struct ipv6_authentication_header
    {
        /** Next header type */
        uint8_t nextHeader;
        /** The length of this Authentication Header in 4-octet units, minus 2.
         * For example, an AH value of 4 equals: [ 3×(32-bit fixed-length AH
         * fields) + 3×(32-bit ICV fields) − 2 ] and thus an AH value of 4 means
         * 24 octets */
        uint8_t headerLen;
        /** Reserved bytes, all zeros */
        uint16_t reserved;
        /** Arbitrary value which is used (together with the destination IP
         * address) to identify the security
         * association of the receiving party */
        uint32_t securityParametersIndex;
        /** A monotonic strictly increasing sequence number (incremented by 1
         * for every packet sent) */
        uint32_t sequenceNumber;
    };

    /**
     * A c'tor for creating a new IPv6 authentication header extension object
     * not bounded to a packet. Useful for adding new extensions to an IPv6
     * layer with IPv6Layer#addExtension()
     ...
     * integrity check value (ICV) data for this extension. Notice this pointer
     * is read-only and its content isn't modified in any way
     ...
     * return pointer points directly to the data, so every change will modify
    ...
    /**
     * @return A pointer to the buffer containing the integrity check value
     * (ICV) for this extension. Notice that any change in this buffer will lead
     * to a change in the extension data
     */
