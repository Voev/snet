...
enum LinkLayerType
{
     ...
     * framing, as per section 4.3.1 of RFC 1547
     */
     ...
     * 0xff address byte replaced by a
    ...
    /** Per Joshua Wright <jwright@hasborg.com>, formats for Z-Wave RF profiles
       R1 and R2 captures */
    LINKTYPE_ZWAVE_R1_R2 = 261,
    /** Per Joshua Wright <jwright@hasborg.com>, formats for Z-Wave RF profile
       R3 captures */
...
public:
     ...
     * elsewhere). This constructor is usually used when packet is captured
     * using a packet capturing engine (like libPcap. WinPcap, Npcap, PF_RING,
     * etc.). The capturing engine allocates the raw data memory and give the
     * user a pointer to it + a timestamp it has arrived to the device
     * @param[in] pRawData A pointer to the raw data
     * @param[in] rawDataLen The raw data length in bytes
     * @param[in] timestamp The timestamp packet was received by the NIC (in
     * usec precision)
     * @param[in] deleteRawDataAtDestructor An indicator whether raw data
     * pointer should be freed when the instance is freed or not. If set to
     * 'true' than pRawData will be freed when instanced is being freed
     * @param[in] layerType The link layer type of this raw packet. The default
     * is Ethernet
     */
    ...
    /**
     * Assignment operator overload for this class. When using this operator on
     * an already initialized RawPacket instance, the original raw data is freed
     * first. Then the other instance is copied to this instance, the same way
    ...
    /**
     * @brief Clones the current packet. Caller is responsible for deallocation
     * of the memory.
     * @return A pointer to the new RawPacket object which is a clone of this
     * object
     */
    virtual RawPacket* clone() const;

    /**
     * @return RawPacket object type. Each derived class should return a
     * different value
     */
     ...
     * @param[in] frameLength When reading from pcap files, sometimes the
     * captured length is different from the actual packet length. This
     * parameter represents the packet length. This parameter is optional, if
     * not set or set to -1 it is assumed both lengths are equal
    ...
    /**
     * Initialize a raw packet with data. The main difference between this
     * method and setRawData() is that setRawData() is meant for replacing the
     * data in an existing raw packet, whereas this method is meant to be used
     * right after constructing a raw packet using the default c'tor, before
     * setting any data
     ...
     * constructor or using the assignment operator. Returns false otherwise,
     * for example: if the instance was created using the default constructor or
     * clear() was called
     */
    ...
    /**
     * Clears all members of this instance, meaning setting raw data to nullptr,
     * raw data length to 0, etc. Currently raw data is always freed, even if
     * deleteRawDataAtDestructor was set to 'false'
     * @todo deleteRawDataAtDestructor was set to 'true', don't free the raw
     * data
     ...
     * end of the current data. This means that the method assumes this memory
     * was already allocated by the user. If it isn't the case then this method
     * will cause memory corruption
     * @param[in] dataToAppend A pointer to the data to append to current raw
     * data
     * @param[in] dataToAppendLen Length in bytes of dataToAppend
     */
    virtual void appendData(const uint8_t* dataToAppend,
     ...
     * data to the end. This means that the method assumes this memory was
    ...
    /**
     * Remove certain number of bytes from current raw data buffer. All data
     * after the removed bytes will be shifted back
     ...
     * allocated buffer to the new one. This method can become useful if the
     * user wants to insert or append data to the raw data, and the previous
     * allocated buffer is too small, so the user wants to allocate a larger
     * buffer and get RawPacket instance to point to it
