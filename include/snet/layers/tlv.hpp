...
public:
    /**
     * A c'tor for this class that gets a pointer to the TLV record raw data
     * (byte array)
    ...
    /**
     * A copy c'tor for this class. This copy c'tor doesn't copy the TLV data,
     * but only the pointer to it, which means that after calling it both the
     * old and the new instance will point to the same TLV raw data
    ...
    static bool canAssign(const uint8_t* recordRawData, size_t tlvDataLen)
    {
        return recordRawData != nullptr &&
               tlvDataLen >= (sizeof(TLVRawData::recordType) +
                              sizeof(TLVRawData::recordLen));
    }
     ...
     * point to the same data, or if they point to different data but their
     * total size is equal and the raw data they both contain is similar.
     ...
     * integer.<BR> Notice this return value is a copy of the data, not a
     * pointer to the actual data
     * @param[in] offset The offset in the record data to start reading the
     * value from. Useful for cases when you want to read some of the data that
     * doesn't start at offset 0. This is an optional parameter and the default
     * value is 0, meaning start reading the value at the beginning of the
     * record data
     ...
     * @param[in] valueOffset An optional parameter that specifies where to
     * start setting the record data (default set to 0). For example: if record
     * data is 20 bytes long and you only need to set the 4 last bytes as
     * integer then use this method like this: setValue<int>(num, 16)
...
public:
     ...
     * record. If tlvDataBasePtr is nullptr or tlvDataLen is zero the returned
     ...
     * next record is out of bounds of the byte stream, a logical null instance
     * of TLVRecordType will be returned, meaning TLVRecordType.isNull() will
     * return true
     */
                                   ...
                                   size_t tlvDataLen) const
    {
        ...
        if (!TLVRecordType::canAssign(
                record.getRecordBasePtr() + record.getTotalSize(),
        ...
        // resRec pointer is out-bounds of the TLV records memory
        if (!resRec.isNull() &&
            resRec.getRecordBasePtr() + resRec.getTotalSize() >
                tlvDataBasePtr + tlvDataLen)
            resRec.assign(nullptr);
     ...
     * @param[in] tlvDataBasePtr A pointer to the TLV data byte stream
     * @param[in] tlvDataLen The TLV data byte stream length
     * @return An instance of type TLVRecordType that contains the result
     * record. If record was not found a logical null instance of TLVRecordType
     * will be returned, meaning TLVRecordType.isNull() will return true
     */
     ...
     * the cached number. This implies that if there is a change in the number
     * of records, it's the user's responsibility to call changeTLVRecordCount()
     * with the record count change
     * @param[in] tlvDataBasePtr A pointer to the TLV data byte stream
     * @param[in] tlvDataLen The TLV data byte stream length
