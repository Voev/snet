...
TEST_P(CertificateSerializeTest, DeserializeSerialize)
{
    ...
    MetaInfo metaInfo{};
    metaInfo.version = ProtocolVersion::TLSv1_2;

    Certificate certificate;
    ASSERT_NO_THROW(certificate = Certificate::deserialize(param, metaInfo));

    Session session(recordPool_);
    ASSERT_NO_THROW(session.processCertificate(certificate));

    std::get<TLSv1Certificate>(certificate.message).certList[0].cert = session.getServerCert();

    ASSERT_NO_THROW(serializedLength = certificate.serialize(serialized, session));
    serialized.resize(serializedLength);

    //snet::utils::compare_vectors(serialized, param);
