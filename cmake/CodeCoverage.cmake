...
#
# To add coverage targets, such as calling `make coverage` to generate the actual
...
# target_code_coverage(theExe) # As an executable target, adds the 'coverage-theExe' target (instrumentation already added via global anyways) for generating code coverage reports.
# ~~~
#
...
# add_executable(theExe main.cpp)
# target_link_libraries(theExe PRIVATE theLib)
# target_code_coverage(theExe) # As an executable target, adds the 'coverage-theExe' target and instrumentation for generating code coverage reports.
# ~~~
#
...
# ~~~
#
# Example 3: Target added to the 'coverage' and 'coverage-all' targets
#
# ~~~
# add_code_coverage_all_targets(EXCLUDE test/*) # Adds the 'coverage-all' target set and sets it to exclude all files in test/ folders.
#
# add_executable(theExe main.cpp non_covered.cpp)
# target_code_coverage(theExe AUTO ALL EXCLUDE non_covered.cpp test/*) # As an executable target, adds to the 'coverage' and coverage-all' targets, and the reports will exclude the non-covered.cpp file, and any files in a test/ folder.
# ~~~
...
set_property(GLOBAL PROPERTY JOB_POOLS coverage_serial_pool=1)
...
if(ENABLE_CODE_COVERAGE AND NOT CODE_COVERAGE_ADDED)
  ...
  elseif(CMAKE_C_COMPILER_ID MATCHES "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES
    ...
    # Targets
    add_custom_target(coverage-clean COMMAND ${LCOV_PATH} --directory
...
# GCOV/LCOV:
# coverage : Generates HTML code coverage report for every target added with 'AUTO' parameter.
# coverage-${TARGET_NAME} : Generates HTML code coverage report for the associated named target.
# coverage-all : Generates HTML code coverage report, merging every target added with 'ALL' parameter into a single detailed report.
#
# LLVM-COV:
# coverage : Generates HTML code coverage report for every target added with 'AUTO' parameter.
# coverage-report : Generates HTML code coverage report for every target added with 'AUTO' parameter.
# coverage-${TARGET_NAME} : Generates HTML code coverage report.
# coverage-report-${TARGET_NAME} : Prints to command line summary per-file coverage information.
# coverage-export-${TARGET_NAME} : Exports the coverage report to a JSON file.
# coverage-show-${TARGET_NAME} : Prints to command line detailed per-line coverage information.
# coverage-all : Generates HTML code coverage report, merging every target added with 'ALL' parameter into a single detailed report.
# coverage-all-report : Prints summary per-file coverage information for every target added with ALL' parameter to the command line.
# coverage-all-export : Exports the coverage report to a JSON file.
#
# Required:
...
# AUTO - Adds the target to the 'coverage' target so that it can be run in a batch with others easily. Effective on executable targets.
# ALL - Adds the target to the 'coverage-all' and 'coverage-all-report' targets, which merge several executable targets coverage data to a single report. Effective on executable targets.
# EXTERNAL - For GCC's lcov, allows the profiling of 'external' files from the processing directory
# COVERAGE_TARGET_NAME - For executables ONLY, changes the outgoing target name so instead of `coverage-${TARGET_NAME}` it becomes `coverage-${COVERAGE_TARGET_NAME}`.
...
# PRE_ARGS <ARGUMENTS> - For executables ONLY, prefixes given arguments to the associated coverage-* executable call ($<PRE_ARGS> coverage-*)
# ARGS <ARGUMENTS> - For executables ONLY, appends the given arguments to the associated coverage-* executable call (coverage-* $<ARGS>)
# ~~~
  ...
  if(ENABLE_CODE_COVERAGE)
    ...
    if(target_type STREQUAL "SHARED_LIBRARY" AND target_code_coverage_ALL)
      if(CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang"
         OR CMAKE_CXX_COMPILER_ID MATCHES "(Apple)?[Cc]lang")
        add_custom_target(
          coverage-run-${target_code_coverage_COVERAGE_TARGET_NAME}
          COMMAND
            ${CMAKE_COMMAND} -E echo "-object=$<TARGET_FILE:${TARGET_NAME}>" >>
            ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list
          DEPENDS ${TARGET_NAME})

        if(NOT TARGET coverage-libs)
          message(
            FATAL_ERROR
    ...
    if(target_type STREQUAL "EXECUTABLE")
      if(CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang"
        ...
        add_custom_target(
          ...
          COMMAND
            ...
            >> ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/profraw.list
          JOB_POOL coverage_serial_pool
          DEPENDS coverage-libs ${TARGET_NAME})

        # Merge the generated profile data so llvm-cov can process it
        add_custom_target(
          coverage-processing-${target_code_coverage_COVERAGE_TARGET_NAME}
          COMMAND
          ...
          DEPENDS coverage-run-${target_code_coverage_COVERAGE_TARGET_NAME})
        ...
        add_custom_target(
          coverage-show-${target_code_coverage_COVERAGE_TARGET_NAME}
          COMMAND
          ...
          DEPENDS coverage-processing-${target_code_coverage_COVERAGE_TARGET_NAME})

        # Print out a summary of the coverage information to the command line
        add_custom_target(
          coverage-report-${target_code_coverage_COVERAGE_TARGET_NAME}
          COMMAND
        ...
        add_custom_target(
          coverage-export-${target_code_coverage_COVERAGE_TARGET_NAME}
          COMMAND
        ...
        add_custom_target(
          coverage-${target_code_coverage_COVERAGE_TARGET_NAME}
          COMMAND
            ${LLVM_COV_PATH} show $<TARGET_FILE:${TARGET_NAME}>
            -instr-profile=${target_code_coverage_COVERAGE_TARGET_NAME}.profdata
      ...
      elseif(CMAKE_C_COMPILER_ID MATCHES "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES
        ...
        if(${CMAKE_VERSION} VERSION_LESS "3.17.0")
          add_custom_target(
            coverage-capture-${target_code_coverage_COVERAGE_TARGET_NAME}
        ...
        add_custom_target(
          ...
          COMMAND
            ...
            ${COVERAGE_INFO}
          DEPENDS coverage-capture-${target_code_coverage_COVERAGE_TARGET_NAME})
      endif()

      add_custom_command(
        TARGET coverage-${target_code_coverage_COVERAGE_TARGET_NAME}
        POST_BUILD
        COMMAND ;
        COMMENT
      ...
      if(target_code_coverage_AUTO)
        ...
        add_dependencies(coverage coverage-${target_code_coverage_COVERAGE_TARGET_NAME})
      ...
      if(target_code_coverage_ALL)
        if(NOT TARGET coverage-all-processing)
          message(
            FATAL_ERROR
              "Calling target_code_coverage with 'ALL' must be after a call to 'add_code_coverage_all_targets'."
          )
        ...
        add_dependencies(coverage-all-processing
                         coverage-run-${target_code_coverage_COVERAGE_TARGET_NAME})
      endif()
    endif()
  endif()
...
# Adds the 'coverage-all' type targets that calls all targets added via
...
# reports. Also adds the coverage-all-capture Generates an all-merged.info file, for
...
function(add_code_coverage_all_targets)
  ...
  if(ENABLE_CODE_COVERAGE)
      ...
      if(WIN32)
        add_custom_target(
          ...
          COMMAND
            ...
            ${EXCLUDE_REGEX}
          DEPENDS coverage-all-processing)
      else()
        add_custom_target(
          coverage-all-report
          COMMAND
    ...
    elseif(CMAKE_C_COMPILER_ID MATCHES "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES
      ...
      add_custom_target(coverage-all-processing COMMAND ;)
