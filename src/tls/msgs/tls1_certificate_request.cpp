...
void TLSv1CertificateRequest::deserialize(nonstd::span<const uint8_t> input)
{
    utils::DataReader reader("CertificateRequest", input);

    casket::ThrowIfTrue(reader.remaining_bytes() < 4, "CertificateRequest: bad message");
...
size_t TLSv1CertificateRequest::serialize(nonstd::span<uint8_t> output) const
{
    /// @todo: support it.
    (void)output;
    return 0;
