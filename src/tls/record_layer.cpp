...
{
    record->ciphertext_ = doTLSv1AeadProcess(cipherCtx, record->getType(), seq, key, iv, record->plaintext_, true);
...
{
    auto ciphertext =
        nonstd::span<uint8_t>(const_cast<uint8_t*>(record->ciphertext_.data()), record->ciphertext_.size());
    record->plaintext_ = doTLSv1AeadProcess(cipherCtx, record->getType(), seq, key, iv, ciphertext, false);
    record->isDecrypted_ = true;
}
                                                  ...
                                                  nonstd::span<uint8_t> out, bool encrypt)
{
    ...
    if (aead_)
    {
        ...
        if (mode == EVP_CIPH_GCM_MODE)
        {
            ...
            crypto::ThrowIfFalse(0 <
                                 EVP_CipherInit(cipherCtx, nullptr, key.data(), nullptr, static_cast<int>(encrypt)));
        }
        else if (mode == EVP_CIPH_CCM_MODE)
        {
        ...
        else
        {
            ...
            std::copy_n(in.begin(), recordIvSize - iv.size(), nonce.begin() + iv.size());
            crypto::ThrowIfFalse(
                0 < EVP_CipherInit(cipherCtx, nullptr, key.data(), nonce.data(), static_cast<int>(encrypt)));
        }
