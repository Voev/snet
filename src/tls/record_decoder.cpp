...
RecordDecoder::RecordDecoder(
    CipherSuite cs, std::span<const uint8_t> macKey, const std::vector<uint8_t>& encKey, const std::vector<uint8_t>& iv)
    : cipherSuite_(cs)
...
void RecordDecoder::tls13_update_keys(const std::vector<uint8_t>& newkey, const std::vector<uint8_t>& newiv) {
...
void RecordDecoder::tls13_decrypt(tls::RecordType rt, std::span<const uint8_t> in, std::vector<uint8_t>& out) {
    ...
    if (EVP_CIPHER_CTX_get_mode(cipher_) == EVP_CIPH_CCM_MODE) {
        tls::ThrowIfFalse(0 < EVP_CIPHER_CTX_ctrl(cipher_, EVP_CTRL_AEAD_SET_TAG, tag.size(), const_cast<uint8_t*>(tag.data())));
    }

    tls::ThrowIfFalse(0 < EVP_DecryptInit_ex(cipher_, nullptr, nullptr, writeKey_.data(), aead_nonce.data()));
    ...
    if (EVP_CIPHER_CTX_get_mode(cipher_) == EVP_CIPH_GCM_MODE) {
        tls::ThrowIfFalse(0 < EVP_CIPHER_CTX_ctrl(cipher_, EVP_CTRL_GCM_SET_TAG, tag.size(), const_cast<uint8_t*>(tag.data())));
    }

    tls::ThrowIfFalse(0 < EVP_DecryptFinal(cipher_, nullptr, &x));
}

void RecordDecoder::tls_decrypt(RecordType rt, int version, std::span<const uint8_t> in, std::vector<uint8_t>& out) {
    ...
    if (cipherSuite_.isAEAD()) {
        ...
        memcpy(aead_nonce + implicitIv_.size(), in, 12 - implicitIv_.size());
        in += 12 - implicitIv_.size();
        inl -= 12 - implicitIv_.size();*/

        tls::ThrowIfFalse(0 < EVP_DecryptInit(cipher_, nullptr, writeKey_.data(), aead_nonce));

        /*
           Then tag is always 16 bytes, as per:
           https://tools.ietf.org/html/rfc5116#section-5.2
        */
        //EVP_CIPHER_CTX_ctrl(cipher_, EVP_CTRL_GCM_SET_TAG, 16, in + (inl - 16));
        //inl -= 16;
    ...
    */
         ...
         * non-stream ciphers. */
        /*if (d->version >= tls::ProtocolVersion::TLSv1_1 && EVP_CIPHER_CTX_get_block_size(cipher_) > 1) {
            uint32_t blk = EVP_CIPHER_CTX_get_block_size(cipher_);
            if (blk <= *outl) {
                *outl -= blk;
                memmove(out, out + blk, *outl);
            } else {
                utils::ThrowIfTrue(true, "Block size greater than Plaintext!");
            }
        ...
        *outl = inl;
        ...
        if (d->version == ProtocolVersion::SSLv3_0) {
            ...
            if (d->version >= ProtocolVersion::TLSv1_1 && EVP_CIPHER_CTX_get_block_size(cipher_) > 1) {
                uint32_t blk = EVP_CIPHER_CTX_get_block_size(cipher_);
                if (blk <= *outl) {
                    *outl -= blk;
                    memmove(out, out + blk, *outl);
...
void RecordDecoder::tls_check_mac(
    RecordType rt, int ver, uint8_t* data, uint32_t datalen, uint8_t* iv, uint32_t ivlen, uint8_t* mac) {
...
void RecordDecoder::ssl3_check_mac(RecordType rt, int ver, uint8_t* data, uint32_t datalen, uint8_t* mac) {
