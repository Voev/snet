...
void ConfigParser::parse(const std::string& filename)
{
    ...
    while (std::getline(file, line))
    {
        ...
        if (length > 0)
        {
            const auto pos = std::find_if(line.begin(), line.end(), [this](char ch) { return ch == '='; });
...
ConfigParser::Sections ConfigParser::getSections() const
{
    return sections_;
}

ConfigParser::Section ConfigParser::getSectionBody(const std::string& section) const
{
