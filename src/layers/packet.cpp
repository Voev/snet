                          ...
                          OsiModelLayer parseUntilLayer)
{
    ...
    while (curLayer != nullptr &&
           (parseUntil == UnknownProtocol ||
            !curLayer->isMemberOfProtocolFamily(parseUntil)) &&
    ...
    {
        // find if there is data left in the raw packet that doesn't belong to
        // any layer. In that case it's probably a packet trailer. create a
...
bool Packet::insertLayer(Layer* prevLayer, Layer* newLayer, bool ownInPacket)
{
    ...
    if (m_RawPacket->getRawDataLen() + newLayerHeaderLen > m_MaxPacketLen)
    {
        ...
        if (m_RawPacket->getRawDataLen() + newLayerHeaderLen >
            m_MaxPacketLen * 2)
                         ...
                         size_t numOfBytesToExtend)
{
    ...
    if (m_RawPacket->getRawDataLen() + numOfBytesToExtend > m_MaxPacketLen)
    {
        ...
        // reallocate to maximum value of: twice the max size of the packet or
        // max size + new required length
        if (m_RawPacket->getRawDataLen() + numOfBytesToExtend >
            m_MaxPacketLen * 2)
            reallocateRawData(m_RawPacket->getRawDataLen() +
                              numOfBytesToExtend + m_MaxPacketLen);
        else
            reallocateRawData(m_MaxPacketLen * 2);
    }
    ...
    while (curLayer != nullptr)
    {
        ...
        size_t headerLen = curLayer->getHeaderLen() +
                           (curLayer == layer ? numOfBytesToExtend : 0);
        dataPtr += headerLen;
        curLayer = curLayer->getNextLayer();
    }
