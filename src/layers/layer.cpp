...
bool Layer::isMemberOfProtocolFamily(
    ProtocolTypeFamily protocolTypeFamily) const
{
...
bool Layer::extendLayer(int offsetInLayer, size_t numOfBytesToExtend)
{
    ...
    if (m_Packet == nullptr)
    {
        ...
        memcpy(newData + offsetInLayer + numOfBytesToExtend,
               m_Data + offsetInLayer, m_DataLen - offsetInLayer);
        delete[] m_Data;
        m_Data = newData;
