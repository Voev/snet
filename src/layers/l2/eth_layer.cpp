...
void EthLayer::parseNextLayer()
{
    ...
    switch (static_cast<EtherType>(be_to_host(hdr->etherType)))
    {
    case EtherType::IP:
    {
...
void EthLayer::computeCalculateFields()
{
    ...
    {
    case IPv4:
        getEthHeader()->etherType = host_to_be(static_cast<uint16_t>(EtherType::IP));
        break;
    case IPv6:
        getEthHeader()->etherType = host_to_be(static_cast<uint16_t>(EtherType::IPV6));
        break;
    case ARP:
        getEthHeader()->etherType = host_to_be(static_cast<uint16_t>(EtherType::ARP));
        break;
    case VLAN:
        getEthHeader()->etherType = host_to_be(static_cast<uint16_t>(EtherType::VLAN));
        break;
    default:
        return;
    }
