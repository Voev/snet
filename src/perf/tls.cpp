...
int tls_init(void) {
    ...
    if (s_user_params.mode == MODE_SERVER) {
        ...
        if (!ctx) {
            snet::log::error("Unable to create SSL context");
            goto error_ctx_not_allocated;
        }

        if (!add_keys_and_certificates(ctx)) {
            snet::log::error("Unable to add keys and certificates");
            goto error_free_ctx;
        }
    } else {
    ...
    if  (!set_tls_version_and_ciphers(ctx)) {
        snet::log::error("Unable protocol and cipher");
        goto error_free_ctx;
    }
    ...
    do {
        ...
        if (ret <= 0 && !g_b_exit) {
            ...
            if (IS_TLS_ERR_WANT_RW(err) || err == SSL_ERROR_WANT_CONNECT) {
                snet::log::debug("tls_connect_or_accept waiting for socket");

                int rc = wait_for_single_socket(SSL_get_fd(ssl), TLS_WAIT_WHICH(err));
                if (0 > rc && errno != EINTR) {
                    snet::log::error("Failed to wait for event while establishing TLS");
                } else if (!g_b_exit) {
                    try_again = true;
                }
            } else {
                unsigned long e = ERR_peek_error();
                snet::log::error("Failed to establish, TLS-Error: {} Reason: {}, {}, {}", err,
...
void *tls_establish(int fd) {
    ...
    if (!ssl_ctx) {
        snet::log::error("Failed tls_establish(), no ssl_ctx");
        goto err;
    }

    ssl = SSL_new(ssl_ctx);
    if (!ssl) {
        snet::log::error("Failed SSL_new()");
        goto err;
    }
    if (!SSL_set_fd(ssl, fd)) {
        snet::log::error("Failed SSL_set_fd()");
        goto err;
    }
...
static inline bool set_tls_version_and_ciphers(SSL_CTX *ctx)
{
    ...
    if (!set_tls_1_2_version_and_ciphers(ctx, p_cipher) &&
        ...
        snet::log::error("Unsupported cipher: {}", p_cipher);
        return false;
    }
