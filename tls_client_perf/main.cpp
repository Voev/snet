...
void keylog(const SSL* tls, const char* line)
{
    ( void )tls;
...
class LatencyStat
{
  public:
    ...
    void update(unsigned long dt) noexcept
    {
        if (!dt)
        {
...
void usage() noexcept
{
    std::cout
        ...
        << "                       '1.2', '1.3' or 'any' for both (default: "
           "'1.2')\n"
...
static int do_getopt(int argc, char* argv[]) noexcept
{
           ...
           -1)
    {
        ...
        case 't':
            ...
            {
                std::cerr << "ERROR: too many threads requested" << std::endl;
                exit(2);
            }
            break;
        case 'n':
    ...
    if (g_opt.keylogfile)
    {
        ...
        if (!bio_keylog)
        {
            std::cerr << "Error writing keylog file '" << g_opt.keylogfile
                      << "'" << std::endl;
            return -ENOENT;
        }
    }
    ...
    if (optind != argc && optind + 2 != argc)
    {
        std::cerr << "\nERROR: either 0 or 2 arguments are allowed: "
                  << "none for defaults or address and port." << std::endl;
        usage();
        return -EINVAL;
    }
    if (optind >= argc)
